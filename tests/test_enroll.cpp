...
void register_enroll_finalize_handler(IpcServer& server,
    ...
    server.on("enroll.finalize",
              [&q, &cache, db_dir](const IpcRequest& req,
        ...
        auto entry = cache.get(job_id);
        if (!entry.has_value()) {
            err.code = static_cast<int>(IpcErrorCode::InvalidParams);
            err.message = "enroll.finalize: diarization no longer cached "
                          "(TTL=" + std::to_string(cache.ttl_secs()) + "s)";
            return false;
        }
        if (target_speaker
            >= static_cast<int64_t>(entry->clusters.size())) {
            err.code = static_cast<int>(IpcErrorCode::InvalidParams);
            err.message = "enroll.finalize: target_speaker out of range";
            return false;
        }
        ...
        if (cluster.embedding.empty()) {
            ...
            err.message = "enroll.finalize: empty cluster embedding";
            return false;
        }
        try {
