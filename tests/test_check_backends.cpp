...
//
// Phase E.4.1 — `recmeet-server --check-backends` is a diagnostic CLI flag
// that prints the `ggml: active backend: <name>` banner from
// `src/backend_info.cpp` and exits 0 BEFORE any pid-lock, socket bind, or
// worker thread. These tests fork+exec the actual installed server binary
// at `build/recmeet-server`, capture combined stdout+stderr via popen, and
...
// Resolve the server binary next to the test runner. CTest's working dir
// is the CMake build directory, so `./recmeet-server` works; but be
// defensive and use the absolute path so `ctest --test-dir <other>`
// invocations don't break.
std::string daemon_binary_path() {
    // The test binary lives at <build>/recmeet_tests, the server at
    // <build>/recmeet-server. /proc/self/exe walks us there regardless of
    // CWD.
    std::error_code ec;
    fs::path self = fs::read_symlink("/proc/self/exe", ec);
    if (ec || self.empty()) return "./recmeet-server";
    return (self.parent_path() / "recmeet-server").string();
}
