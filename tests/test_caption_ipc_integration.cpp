...
const std::string CAPTION_SOCK =
    recmeet::test::tmp_path("recmeet_test_caption.sock").string();
...
struct CaptionDaemonSim {
    ...
    explicit CaptionDaemonSim(const std::string& sock = CAPTION_SOCK) : server(sock) {
        unlink(sock.c_str());
...
std::unique_ptr<IpcClient> caption_client(const std::string& sock = CAPTION_SOCK) {
...
TEST_CASE("captions_enabled=true creates captions.vtt with finals only",
    ...
    fs::path meeting_dir = recmeet::test::tmp_path(
        "recmeet_vtt_ipc_" + std::to_string(::getpid()) + "_on");
...
TEST_CASE("captions_enabled=false produces no captions.vtt sidecar",
    ...
    namespace fs = std::filesystem;
    fs::path meeting_dir = recmeet::test::tmp_path(
        "recmeet_vtt_ipc_" + std::to_string(::getpid()) + "_off");
    fs::remove_all(meeting_dir);
    fs::create_directories(meeting_dir);
    fs::path vtt_path = meeting_dir / "captions.vtt";
