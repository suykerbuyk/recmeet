...
// which would otherwise sit silent for 30+ minutes — exactly the silence the
// task exists to close. Short-tag suites (unit tests) stay quiet by default.
...
class ProgressListener : public Catch::EventListenerBase {
  public:
    ...
    void testCaseStarting(const Catch::TestCaseInfo& info) override {
        ...
        // Thread construction is a C++ memory-model sync point — writes here
        // happen-before reads in the heartbeat thread, no explicit acquire/release
        // needed.
  ...
  private:
    void start_heartbeat() {
        ...
        heartbeat_ = std::thread([this] {
            ...
            while (!stop_flag_.load()) {
                ...
                if (cv_.wait_until(lk, next, [this] { return stop_flag_.load(); })) return;
                auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::steady_clock::now() - case_start_).count();
                long rss_kb = recmeet::read_self_rss_kb();  // returns 0 on failure
                // Pre-format into one line so emission is one syscall — concurrent
                // PhaseEcho writes from the test thread cannot interleave mid-line.
                std::string rss_field = (rss_kb > 0)
                    ? "rss=" + std::to_string(rss_kb / 1024) + " MiB"
                    : "rss=?";
    ...
    static std::chrono::seconds default_interval() {
