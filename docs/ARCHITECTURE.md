...
| `Meeting_<ts>.<NN>[_<title>].md` | Always | The meeting note (transcript, summary, action items). `<NN>` is a zero-padded reprocess-attempt counter (`.00`, `.01`, ..., unbounded — `.99 → .100`); see "Reprocess-attempt counter" below |
...
**Reprocess-attempt counter.** The meeting note's `.<NN>` segment (between the timestamp and the optional title) is a zero-padded counter assigned by `recmeet::note_internal::next_attempt_and_migrate()` (declared in `src/note.h`, defined in `src/note.cpp`). On every `write_meeting_note()` call the helper performs a stateless directory scan: it parses the `.<NN>` from every existing `Meeting_<ts>.NN[_*].md` numerically (so `.10 > .2`), picks `max+1` for the new write, and migrates any legacy un-numbered `Meeting_<ts>[_*].md` files for the same timestamp to `.<max+1>`, `.<max+2>`, ... sorted by file `mtime` ascending (filename ascending as the tiebreak — deterministic across hosts even when `cp -a` / syncthing replication collide mtimes). The `++max_seen` step always advances even when a rename fails, so a transient rename failure (ENOSPC, EACCES, syncthing lock) burns its slot and leaves a permanent gap rather than risking a future collision; the next write self-heals any remaining orphans because the scan-and-renumber happens unconditionally on every call. If every attempt file for a timestamp is deleted, the next write naturally resets to `.00` (no persistent sidecar). The `meetings.read_note` IPC handler reads the highest-numbered attempt, falling back to newest-mtime among legacy un-numbered notes when no migration has run yet. See the helper's doc comment in `src/note.h` for the failure contract and `tests/test_note.cpp [note][note-attempts]` for the 12-case behavior matrix.
