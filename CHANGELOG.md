...
- **Dual-source recording (mic + system audio) restored for V2** (`v2-dual-source-recording`, V1-parity gate):
  - The V2 thin client (`recmeet-client`) again records the microphone **and** the system-audio monitor (loopback) together, mixing them to a single mono WAV — restoring V1 behavior that the initial thin-client split dropped to mic-only. Meetings where remote participants are only audible on the monitor source (most video calls) are usable again.
  - **Mixing is offline, once, at Stop** — the two complete capture buffers are averaged via a single `mix_audio()` call (V1-identical), not per-tick in the streaming pump. Per-tick mixing of two independently-clocked `pw_thread_loop`s would zero-pad every 100 ms and accumulate ~11 s/hour of lip-sync drift; the offline mix has zero cumulative drift.
  - **Monitor backend follows V1**: a `.monitor` source name uses the PulseAudio `pa_simple` path directly; any other sink name tries PipeWire `CAPTURE_SINK` and falls back to `pa_simple` (Bluetooth / pipewire-pulse). Auto-detection only ever selects a real sink loopback (`DetectedSources.monitor`), never `mic + ".monitor"`, so local voice is not double-counted.
  - **Graceful degradation**: with `mic_only = false` and no monitor resolved, or a monitor that fails to open / produces too-short audio, the recording continues mic-only with a warning — never a hard failure (V1 parity). `mic_only = true` keeps the single-mic path unchanged. Monitor-only recording is intentionally out of scope (V1 had no such mode).
  - **No server or wire-protocol changes** — entirely tray-local. The mix decision (`finalize_dual_mix`) and the capture-side duration gate (`validate_min_duration`) live in `recmeet_capture` (moved there from `recmeet_core`) so the tray, which cannot link `recmeet_core`, can mix without pulling in the ML stack. Hermetic `[tray][dual]` unit tests cover the mix-at-Stop contract and the non-fatal monitor fallback; the existing `[full-stack][e2e][thin-client]` suite stays green. Live captions stay on the mic source in dual mode (V1 parity).
...
- **Meeting notes now include a `.NN` reprocess-attempt counter in the filename.** The pattern changes from `Meeting_<YYYY-MM-DD>_<HH-MM>[_<title>].md` to `Meeting_<YYYY-MM-DD>_<HH-MM>.<NN>[_<title>].md`. The counter is zero-padded to 2 digits, unbounded upward (`.99 → .100`), sorts oldest-to-newest within a single recording, and increments by one on each reprocess — so every prior attempt's note is preserved for side-by-side comparison instead of being overwritten by the next reprocess. Existing un-numbered notes from earlier recmeet versions are migrated to the new form on the first new write for that recording (sorted by file `mtime` ascending: oldest becomes `.00`, next `.01`, and so on; filename ascending as the tiebreaker for collisions from `cp -a` / syncthing replication). Partial migrations from prior runs (e.g. a transient `fs::rename` failure left some files orphaned) self-heal automatically on the next write — no operator intervention required. The `meetings.read_note` IPC handler returns the highest-numbered attempt when multiple exist; for un-migrated legacy notes it returns the newest by `mtime`. Go-side MCP tools (`recmeet-mcp`, `recmeet-agent`) still pick notes by `mtime` for now — a follow-up task (`meetingdata-attempt-aware`) will align them; until then, manual edits to an older attempt in Obsidian may produce divergent picks between the daemon and the Go tools (the daemon prefers the highest `.NN`; the Go tools prefer the newest `mtime`). Tested end-to-end via `[full-stack][note-attempts]` (real `recmeet-server` subprocess + `process.submit` + `process.reprocess`).
...
- **Meeting notes are now readable by the daemon's own `meetings.read_note` / `process.fetch`.** With the default (unset) `note_dir`, the postprocess pipeline used to set `note_dir = output_dir` as a fallback, which tripped the writer's `YYYY/MM/` sub-bucketing and buried every note at `<meeting_dir>/<YYYY>/<MM>/Meeting_*.md` — a path the non-recursive read paths never entered. The WebUI "view note" silently 404'd and `process.fetch` returned no `.md` artifacts for meetings the daemon itself had just processed. The fallback is removed: default-config notes now land flat in the meeting dir per the documented contract (`note_dir` empty = use `output_dir`), and the readers find them. An explicit `note_dir` (CLI `--note-dir` / config `notes.directory`) still uses the `YYYY/MM/` hierarchy, unchanged. On daemon startup a one-time migration relocates any pre-existing `<meeting_dir>/<YYYY>/<MM>/Meeting_*.md` notes up into the meeting dir (idempotent; never clobbers an existing flat note); the migration count is logged. Tested via the `[note][note-migration]` unit suite and the end-to-end `[full-stack][note-attempts]` test (which now reads the note back via `meetings.read_note` + `process.fetch` rather than inspecting disk).
